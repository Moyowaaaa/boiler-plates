<template>
  <div>
    ...
    <div v-if="!preloaderDone">
      ...
      <div class="transition-overlay" ref="transitionRef">
        <svg
          width="14"
          height="15"
          viewBox="0 0 14 15"
...
const transitionObject: TransitionProps = {
  ...
  onBeforeLeave() {
    ...
    gsap.set(transitionRef.value, { display: "flex", opacity: 1, yPercent: 0 });
  },
  onLeave(el: any, done: any) {
    ...
    tl.to(
      ...
      .to(".page", { opacity: 0, duration: 0.5, ease: "power3.inOut" }, "-=0.9")
      .to(
        transitionRef.value,
        { top: "-100%", duration: 1.5, ease: "expo.inOut" },
        "+=0.5",
      );
  },
  onAfterLeave() {
    gsap.set(transitionRef.value, { display: "none", top: "100%" });
  },
  ...
  onAfterEnter() {
    gsap.set(transitionRef.value, { display: "none", opacity: 0, yPercent: 0 });
  },
};
